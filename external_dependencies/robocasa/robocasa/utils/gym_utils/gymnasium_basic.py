...
def _env_layout_style_ids(name, default):
    raw = os.environ.get(name)
    if raw is None:
        return default
    parsed = json.loads(raw)
    if not isinstance(parsed, list):
        raise ValueError(f"{name} must be a JSON list, got {type(parsed)}")
    normalized = []
    for pair in parsed:
        if not isinstance(pair, (list, tuple)) or len(pair) != 2:
            raise ValueError(f"{name} entries must be [layout_id, style_id], got {pair}")
        normalized.append([int(pair[0]), int(pair[1])])
    if not normalized:
        raise ValueError(f"{name} must contain at least one [layout_id, style_id] pair")
    return normalized
...
def create_env_robosuite(
    ...
    if issubclass(env_class, Kitchen):
        benchmark_layout_and_style_ids = _env_layout_style_ids(
            "ROBOCASA_LAYOUT_STYLE_IDS",
            [[1, 1], [2, 2], [4, 4], [6, 9], [7, 10]],
        )
        benchmark_obj_instance_split = _env_optional_str("ROBOCASA_OBJ_INSTANCE_SPLIT", "B")
        benchmark_randomize_cameras = _env_bool("ROBOCASA_RANDOMIZE_CAMERAS", False)

        env_kwargs.update(
            {
                "layout_ids": None,
                "style_ids": None,
                "layout_and_style_ids": benchmark_layout_and_style_ids,
                "obj_instance_split": benchmark_obj_instance_split,
                "generative_textures": None,
                "randomize_cameras": benchmark_randomize_cameras,
            }
        )
...
class RoboCasaEnv(gym.Env):
    ...
    def get_basic_observation(self, raw_obs):
        ...
        language_override = os.environ.get("ROBOCASA_LANG_OVERRIDE")
        raw_obs["language"] = (
            language_override
